...
function app() {
    ...
    THREE.SetErrno((err, line_offset) => {
        err = err
            .split('\n')
            .filter(x => x.indexOf("ERROR") != -1)
            .map(x => {
                let a = x.split(":");
                let error_text = a.slice(3).join(":");
                let line = parseInt(a[2]) - line_offset;

                return line + ":" + error_text;
            })
            .join('\n');
        console.log("three err:", err);
        dat_errors.push(gui.add({"": err}, ""));
    });

    editor.on("change", (_) => cbs.forEach(cb => {
        dat_errors.forEach(x => gui.remove(x));
        dat_errors = [];
        cb.update_material(editor.getValue());
    }));
